...
# declaring the AWS storage information
input_type = 'nfs'
...
# Getting the list of IP address Based on the Input type
ip_list = None
if input_type == 'nfs':
...
# Getting the list of IP results based on the Scan type
results = None
...
# get the existence storge bucket names
...
# Generate the Bucket name with the random object with help of uuid
def genBucketName():
    return s3_bucket_prefix + str(uuid.uuid4())

# Creating the bucket name based on location
...
#Crete the bucket and client if not existed
...
# sotre the resuls in the json file with encoded
...
# working on s3 bucket based on the Bucket name
...
#generate the file key with the help of time
def genFileKey():
    return time.strftime('%y-%m-%d-%h:%m:%s', time.localtime())

# Store the results in S3 bucket based on the storage type
def storagetype():
    if storage_type == 's3':
